...
- `bin/` — binaries/scripts install.sh plain-copies (not symlinks, not templated) into `~/.local/bin`; drop files in to have them installed. Currently `jwatch`, the body of the `prefix+j` Slurm popup that **both** tmux and herdr bind (see below). Note `~/.local/bin/hsl` is *not* a candidate: herdr-statusline generates that launcher itself.
