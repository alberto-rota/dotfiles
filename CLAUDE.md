...
- it is gated by **`cc_needed()`**, shaped like `brew_needed()`: install a compiler only when something selected has no route *except* a cargo build — which since the section above is `herdr_statusline` and nothing else. Deliberately does not ask `tool_route()` — that route consults `AVAIL_CC`, so it would be circular. apt (`build-essential`) is the only route offered; there is no userland way to put a working C toolchain on a Linux box, and a Homebrew gcc is a multi-hundred-megabyte detour that still leaves rustc looking for `cc`.
...
**`hsl` where there is one, plain `herdr` where there is not — which is every Mac.** `hsl` ships with the herdr-statusline plugin (it is a generated launcher — do **not** put it in `bin/`; the plugin's build step is what writes it, and that build needs cargo — see `tool_deps()` above), and that plugin's manifest is `platforms = ["linux"]`, so `tool_route herdr_statusline` on macOS is `na|linux only` and `hsl` is never installed there at all. The autostart used to test for `hsl` alone, which meant **the answer silently did nothing on every Mac**: everything installed, the file was rendered and linked, and no terminal ever started anything. It now takes the first of `hsl`, `herdr` that exists. The answer is "start my workspace manager when I open a terminal"; the status line is a detail of how that workspace looks, not the thing being asked for, and `herdr` with no arguments launches or attaches to the same persistent session `hsl` wraps.

Both front-ends and every message follow that: the checkbox and the text prompt read **"start herdr at login"**, the summary line names herdr on a Mac and hsl elsewhere, and `print_next_steps()` has three branches (hsl, herdr-with-a-note-about-why, and neither → the autostart no-ops harmlessly). The two loop variables in the script are `unset` on the way out including the early-return path, because this file is *sourced into the user's interactive shell* and anything left behind is in their environment for the rest of the session.
