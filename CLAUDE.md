...
The fallback to plain `herdr` stays anyway, for the window where `~/.local/bin` is not on PATH yet (a first install, before the shell has been re-sourced). The answer is "start my workspace manager when I open a terminal"; the status line is a detail of how that workspace looks, not the thing being asked for, and `herdr` with no arguments launches or attaches to the same persistent session `hsl` wraps — which is also exactly how `bin/hsl` itself degrades when tmux is missing.

Both front-ends and every message follow that: the checkbox and the text prompt read **"start herdr at login"**, and `print_next_steps()` has three branches (hsl, herdr-with-a-note-that-`~/.local/bin`-is-not-on-PATH-yet, and neither → the autostart no-ops harmlessly). The two loop variables in the script are `unset` on the way out including the early-return path, because this file is *sourced into the user's interactive shell* and anything left behind is in their environment for the rest of the session.
...
### `update` either updates or says it did not

**A refused fast-forward is fatal in `cmd_update`, and that is the single most important thing about it.** It used to print a NOTE and install anyway, which made the one command whose entire job is *get the new code* report success while rendering the **old** code — and the way that presents is genuinely baffling. On a machine whose tracked `claude/settings.json` carried a one-line hand edit, `git pull --ff-only` aborts (the incoming commit touched that same file), the NOTE scrolled past inside a screenful of install output, and the prompt, both status bars and even the setup UI's live preview all sat quietly on a commit four back — through *repeated* updates, because every one of them failed the same invisible way. The plan-vs-run split `lib/tools.sh` goes to such lengths to prevent has an update-vs-updated twin, and this was it.

This does **not** contradict install.sh's bootstrap, which deliberately falls through to installing whatever is on disk after a failed refresh. That code's job is to get a machine set up *at all*, where local edits or no network are reasons to use what you have rather than to leave somebody with nothing. By the time `dotfiles update` runs there is already a working install to keep, so stopping costs nothing and carrying on costs you the truth. `--keep-going` restores the old behaviour for the one case that wants it — an offline machine being re-rendered after a template edit, where you know the checkout is stale.

**`update_diagnose()` exists because git's own message is accurate and generic.** Each branch ends in a command to run, and the ordering is deliberate: detached HEAD, then no upstream, then **unreachable remote before anything about the working tree** (no network and no credentials both present as a failed pull on a checkout in perfect shape, where the answer is "try later", not "go and edit your files"), then the two collision cases, then local commits, then an honest "not a cause this CLI recognises".

The collision cases are the ones worth understanding, since they are what actually happens:

- **tracked files edited here *and* changed by the incoming range** — that intersection is exactly what git refuses to overwrite, computed with `comm -12` over `diff --name-only HEAD` (`HEAD`, not a bare `diff`, so staged edits count) and `diff --name-only HEAD..@{u}`. A local edit to a file the range does not touch blocks nothing, so listing every modified file would send you after the wrong ones.
- **untracked files where the range wants to create one** — the renamed-template case. A template that moves between directories arrives as a create, and one leftover copy at the destination stops the whole pull. `herdr/plugins/herdr-statusline/gpu-status.sh.in` → `tmux/gpu-status.sh.in` is a real instance of it.

The pull has already fetched by the time the merge refuses, so `@{u}` is current and both intersections are computed against the real incoming range rather than a stale ref.

**install.sh names locally-modified tracked files on the way out of every run**, which is the other half of the same fix. The failure above is one of *accumulation*: a hand edit sits there invisibly — the install keeps working, everything renders, nothing complains — until the day an incoming commit collides with it. The CLI is loud when that day arrives, but by then the checkout has been frozen for however long. So the NOTE fires on every run, whether or not there is anything to pull, while the edit is still fresh in somebody's memory. Only *tracked* modifications count: untracked files in a checkout are normal and only collide in the narrow renamed-template case above.

**`claude/settings.json` was the file that triggered all of this, and templating it would be the wrong fix.** It is `link()`ed, so it is a symlink into the repo, and a hand edit to it therefore dirties the tracked tree. The tempting answer — render it into `.generated/` so writes land somewhere gitignored — was checked and rejected: Claude Code persists its own state to `~/.claude.json` (and the model per project there), *not* to `~/.claude/settings.json`, so nothing writes that file except a person. Rendering it would trade a problem that only occurs when you deliberately edit it for one where every `dotfiles update` silently overwrites that edit. The two changes above make the hand-edit case loud and self-explaining instead, which is what it needed.
...
The mapping is deliberate, and its three groups are the whole design:

- **primary — everything that is Claude addressing you.** `permission`, `suggestion` and `remember` are all one colour upstream (`#b1b9f9`, the light blue you notice in permission prompts, the autocomplete menu and `#`-memory hints); `promptBorder`, the spinner (`claudeBlue_FOR_SYSTEM_SPINNER`), the brand orange (`claude`) and `clawd_body` join them.
- **secondary — everything that says a MODE is engaged.** `autoAccept`, `skill`, `merged`, `planMode`, `bashBorder`, `ide`. Upstream spends four hues on these (purple, teal, pink, blue) and they collapse to one here on purpose: what matters is *that* a mode is on, and which one is already written beside the indicator.
- **untouched, for the reason `slurm-status.sh.in` draws job state in fixed neon green and yellow.** `error`, `warning`, `success`, the six `diff*` colours, every `*_FOR_SUBAGENTS_ONLY` (they work only by being distinct from *each other*), and `text` / `inverseText` / `inactive` / `subtle` / `background`. A colour that answers a question has to answer it the same way on every machine, whatever accents are set.
